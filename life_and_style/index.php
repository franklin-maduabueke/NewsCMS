...
<link rel="stylesheet" type="text/css" href="<?php echo $prefix;?>stylesheet/life_and_styleLook.css" />
...
?>
	...
	<div id="pageContentHolder">
		<div id="pageContentMainBlock">
			...
			<?php
				//query for top story.
				$sql = sprintf("SELECT Heading, TextContent, ArticleGenID, SCGenID FROM Articles WHERE TopStory=1 AND PublishDate<>'0000-00-00' AND SCGenID IN(SELECT SCGenID FROM Subcategory WHERE CatGenID='%s') ORDER BY PublishDate DESC, AID DESC", PAGE_NAME);
			...
			<div id="pageLeftColumn">
				...
				<?php
					...
					$sql = sprintf("SELECT Heading, TextContent, ArticleGenID, SCGenID FROM Articles WHERE ArticleGenID<>'%s' AND SCGenID IN(SELECT SCGenID FROM Subcategory WHERE CatGenID='%s') AND PublishDate<>'0000-00-00' ORDER BY PublishDate DESC, AID DESC", $topstoryArticleRow['ArticleGenID'], PAGE_NAME);
				...
				<div id="pageLeftColumnBottomHolder">
					...
					<div class="pLColSubCatContentHolder">
						...
						<?php
								...
								$secRelatedResult = $dbConn->query($sql);
								if ($secRelatedResult && $secRelatedResult->num_rows > 0)
								{
									$secRow = $secRelatedResult->fetch_array();
						?>
						<div class="plColSubCatArticleRelated"><a href="<?php echo $prefix;?>processing/redirectToRead.php?sel=<?php echo $secRow['CatGenID'];?>&sc=<?php echo $secRow['SCGenID'];?>&article=<?php echo $secRow['ArticleGenID'];?>"><?php echo substr($secRow['Heading'], 0, 33);?></a></div>
						<?php
								}
						?>
					</div>
