<?php
 ...
 $adToShow = rand(1, 2); //flash or image
	...
	case AD_FLASH:
		...
		$sql = sprintf("SELECT ad.ClientGenID, ad.AdvertType, ad.AdsGenID, fAd.FATID, fAd.FlashAdGenID FROM adverts AS ad JOIN flashads AS fAd ON ad.AdsGenID=fAd.AdsGenID WHERE fAd.FlashAdType=%d AND ad.Duration>=%d AND ad.AdvertType=%d ORDER BY AdvertViewCount", AD_FLASH_BANNER_VERTICAL, time(), AD_FLASH);
...
?>
		<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0" width="160" height="600">
       		 	<param name="movie" value="<?php echo $prefix;?>temps/<?php echo $advertRow['AdsGenID'] . $advertRow['FlashAdGenID'] . ".swf";?>" />
        		<param name="quality" value="high" />
        		<embed src="<?php echo $prefix;?>temps/<?php echo $advertRow['AdsGenID'] . $advertRow['FlashAdGenID'] . ".swf";?>" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="160" height="600"></embed>
      	</object>
<?php
			//update view count.
...
<?php
	...
	<a href="<?php echo $advertRow['ImageAdLink'];?>" style="text-decoration:none; color:#FFFFFF; border:0px" target="_blank">
		<img src="<?php echo $prefix . "temps/" . $advertRow['AdsGenID'] . $advertRow['ImageAdGenID'] . "." . $advertRow['FileFormat'];?>" style="width:160px; height:600px" />
	</a>
<?php
			//update view count.
			$sql = sprintf("UPDATE adverts SET AdvertViewCount = AdvertViewCount + 1 WHERE AdsGenID='%s'", $advertRow['AdsGenID']);
	...
	<a href="#" style="text-decoration:none; color:#FFFFFF; border:0px">
		<img src="<?php echo $prefix;?>images/banner.jpg" style="width:160px; height:600px" />
	</a>
	<?php
		}
	}
