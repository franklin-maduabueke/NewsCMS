	...
	if ($ini)
	{
		...
		if (file_exists("../samples"))
		{
			...
			if ($samples && count($samples) > 0)
			{
				...
				{
					if (strpos($samples[$i], ".jpg") && strpos($samples[$i], "Subcat") == FALSE ) //select only jpg files that do not have the subcategory bar linked.
					{
		...
		<tfoot>
			<tr>
				<td style="text-align:left">
					<?php if ($current_page > 1) { ?><a href="room.php?sel=<?php echo $selectedCategory;?>&user=<?php echo $_SESSION['authentication']; ?>&role=<?php echo $_SESSION['Role'];?>&tsk=select_template&tabName=<?php echo $tabName; ?>&page_no=<?php echo $current_page - 1; ?>">&laquo; Previous</a><?php } ?>
				</td>
				...
				<td style="padding-left:0px; padding-right:10px; text-align:right">
					<?php if ($current_page < $totalPages) { ?><a href="room.php?sel=<?php echo $selectedCategory;?>&user=<?php echo $_SESSION['authentication']; ?>&role=<?php echo $_SESSION['Role'];?>&tsk=select_template&tabName=<?php echo $tabName; ?>&page_no=<?php echo $current_page + 1; ?>">Next &raquo;</a><?php } ?>
				</td>
			</tr>
		</tfoot>
		<tbody>
			...
			<tr>
				...
				<td style="border-right:0px; padding-left:0px; text-align:center;">
					<a href="room.php?sel=<?php echo $selectedCategory;?>&user=<?php echo $_SESSION['authentication']; ?>&role=<?php echo $_SESSION['Role'];?>&tsk=selected_template_set_sections&sample=<?php echo basename($foundTemplates[$i],".jpg") . ".php"; ?>&tabName=<?php echo $tabName; ?>">Use Template</a>
				</td>
			</tr>
			<?php
						$drawCount++;
