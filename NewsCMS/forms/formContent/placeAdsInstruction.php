<?php
	...
	$placeAdsTxtContent = "Write about us content here!";
	$placeAdsTxtPath = NULL;
	//get the about us text location
	$ini = parse_ini_file("../config/app.ini", TRUE);
	
	if ($ini && array_key_exists("SITE_MISC", $ini) && array_key_exists("PlaceAdsFile", $ini['SITE_MISC']))
	{
		$placeAdsTxtPath = $ini['SITE_MISC']['PlaceAdsFile'];
		
		if (file_exists($ini['SITE_MISC']['PlaceAdsFile']))
		{
			$placeAdsTxtContent = file_get_contents($ini['SITE_MISC']['PlaceAdsFile']);
		}
		else
		{
			//create the file in that path.
			$fHnd = fopen($ini['SITE_MISC']['PlaceAdsFile'], "x");
			
			if ($fHnd)
			{
				fwrite($fHnd, $placeAdsTxtContent);
				fclose($fHnd);
			}
		}
	}
...
<div style="width:997px; height:auto; min-height:400px;background-color:#FFFF8A; border:1px solid #FEE956; text-align:left">
	<input type="hidden" value="<?php echo $placeAdsTxtPath; ?>" name="pathToFile" />
	<?php
		...
		$articleEditor->editor("articleEditor", $placeAdsTxtContent, $toolbar);
	?>
	<!--use info -->
</div>
</form>
